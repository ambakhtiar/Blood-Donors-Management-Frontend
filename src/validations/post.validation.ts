...
const contactNumberSchema = z
  .string({ required_error: "A contact number is required" })
...
const bloodFindingSchema = z
  .object({
    ...
    bloodBags: z.coerce
      .number({ required_error: "Number of blood bags is required" })
    ...
    reason: z
      .string({ required_error: "Please specify the reason for needing blood" })
      .trim()
...
const bloodDonationSchema = z.object({
  ...
  title: z
    .string({ required_error: "Title is required for the donation post" })
    .trim()
  ...
  donationTime: z
    .string({ required_error: "Please specify when you are available to donate" })
...
const helpingSchema = z.object({
  ...
  images: imageUrlSchema,
  reason: z
    .string({ required_error: "Please specify why funds are needed" })
    .trim()
    .min(10, "Reason must be at least 10 characters long")
    .max(1000, "Reason description is too long"),
  medicalIssues: z
    .string({ required_error: "Please describe the medical issues" })
    .trim()
    .min(10, "Medical description must be at least 10 characters long")
    .max(2000, "Medical description is too long"),
  contactNumber: contactNumberSchema,
  targetAmount: z.coerce
    .number({ required_error: "Target fund amount is required" })
    .min(100, "Target amount must be at least 100 BDT"),
  location: z
    .string({ required_error: "Patient location/hospital name is required" })
    .trim()
    .min(3, "Location description must be at least 3 characters"),
  bkashNagadNumber: z
    .string({ required_error: "bKash/Nagad number is required for receiving funds" })
